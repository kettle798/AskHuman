...
Watch 规格见 `docs/specs/im-watch.md`。订阅持久化在 `~/.askhuman/state/watch.json`，daemon 重启后尽量继续编辑原卡。状态卡展示 Agent 工作中、空闲、等待回答或已结束，以及跨 Turn 累计、扣除真正 idle 的工作时长和最近活动；状态变化、提问和回答会唤醒引擎。
