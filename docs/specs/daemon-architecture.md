...
1. **长连接无法独占共享**（见 `docs/TODO.md` 问题 1）。钉钉官方限制「同一 client-id 同一时刻只允许一条 Stream」，飞书长连接同理。当前每个 `AskHuman` 进程各自开一条长连接，**连续快速 / 并发提问时**多条连接会相互抢消息，可能把用户回复投递到错误的进程。这是「每进程各自持连接」的结构性缺陷，给单个渠道打补丁（文件锁等）只能缓解，无法统一根治。
