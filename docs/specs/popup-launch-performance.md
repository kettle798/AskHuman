...
- **隔离 daemon（关键）**：harness 在一个**临时 `HOME`**（temp 目录）下运行**自己的独立 daemon**——因为 socket/lock/perf.log/agents 等全在 `$HOME/.askhuman` 下，重定向 `HOME` 即把 CLI/daemon/helper 全部隔离。**绝不触碰用户真实 daemon 与在途提问**，也使 `--cold`（每次跑前停 daemon）安全。结束时停隔离 daemon 并删临时目录（`--keep-home` 可保留）。
...
- 跑完按 `perf_id` 聚合隔离 `HOME` 下的 `perf.log`，算每段 delta 的 **中位数 / p90 / min / max**。
- 默认（warm）先 `daemon start` 起隔离 daemon 并保活全程；`--cold` 则每次跑前 `daemon stop --force` 单独测 daemon 冷启动。默认丢弃前 `--warmup` 个样本。
- **隔离 vs 真实 daemon**：用户真实 daemon 因 agent 生命周期 1s 轮询 / IM 长连 / 与本 agent 抢答而**有竞争负载**，会污染读数；隔离 daemon 干净可复现。实测同机隔离比真实 daemon 快 ~100ms+（real-HOME e2e+spawn ~586ms → 隔离 ~474ms）——故基线与对比**统一在隔离 daemon 下做**。注意：隔离 daemon 用**默认 config（无 IM）**，故 `daemon`/`im_attach` 段 ≈0；这与「优化页面加载」无关。
- 用法：
  ...
  - 关键参数：`--threshold P`（端到端 p90 劣化阈值，默认 20%）、`--runs`、`--warmup`、`--cold`、`--timeout`、`--json`、`--keep-home`。
...
> 正式基线见 `docs/perf/baseline.json`（`--save-baseline` 生成）。隔离 daemon 用默认 config（无 IM）。
