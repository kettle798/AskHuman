...
2026-07-23 补充：Codex 桌面版会在用户创建任务前运行 Suggested prompts 的 task-suggestion generator。
该角色同样不进入 AskHuman 交互协议，因此 Codex 的第一句按用户确认做最小替换：

```text
**This protocol does not apply to subagents or task-suggestion generators; if you are either, do not use AskHuman.**
```

`cli_reference_for(agent)` / `mcp_reference_for(agent)` 从共享正文按目标 Agent 生成规则；只有 Codex 使用
上述替换句，Claude、Cursor 与 Grok 仍使用原 subagent 句，第二条委派规则对四家保持一致。这里不匹配
Suggested prompts 的具体 prompt；Codex MCP 另从可信 turn metadata 精确拦截 `thread_source=system`
（见 `mcp.md`）。
...
10. 只有 Codex CLI / MCP Rules 含 task-suggestion generator 例外；Claude、Cursor 与 Grok 产物不含该句。
