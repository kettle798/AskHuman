...
1. **单独使用**：用户 `npm i -g askhuman` 后在终端直接用 `AskHuman ...`；或从 GitHub Release 下载。
...
| D4 | 主包名 | `askhuman`（非 scoped，与命令名 `AskHuman` 对应；`humaninloop` 因与 `human-in-loop` 太像被拒，故弃用） |
...
2. 在 macOS/Windows/Linux 任一机器 `npm i -g askhuman` 后，`AskHuman` 命令可用，仅安装当前平台对应的一个子包。
