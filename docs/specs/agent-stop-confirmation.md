...
- 默认 CLI / MCP 提示词在既有“结束前必须询问、必须得到可以结束且没有更多任务的确认”规则后，
  要求 Agent 在用户同意结束后把 `[user_confirmed_end_turn]` 作为最终输出的最后一个非空行。
  该规则与 Stop 开关无关，避免切换开关时联动更新提示词。
- Stop Hook 仅在自然 Stop 的原始最后回复以独立尾行精确命中该标记时静默放行并移除标记；
  出现在正文中、前后带其它字符或后面仍有内容都不命中。Agent 忘记标记时安全退化为再次发 Stop 卡。
...
- `[user_confirmed_end_turn]` 仅精确独立尾行命中；正文/前后缀/尾后有内容不命中；命中后先剥离再展示；
