...
>
> 渠道：飞书（P1 首发）+ Telegram + Slack（多渠道扩展，`docs/plans/im-watch-channels.md`）；
> 钉钉 PoC 已通过（模板 / 渲染器 / 高频更新 / 按钮回调全实测，结论见计划 §4；M4 接入引擎）。
> 引擎与语义（签名门控 / 跟底 / 终态 / 持久化）全渠道一致，差异仅在渲染与传输
> （见「渠道差异」节）。
...
| 钉钉 | `PUT /v1.0/card/instances` 更新公有变量（已有） | 实测 2s×60 连发零频控，p50 ≈60–95ms | 模板内按钮（Stream 回调） | **PoC 通过**（专用模板已建；M4 接入引擎） |
...
- **渠道门控**（`watch::channel_supported`）：飞书 / Telegram / Slack 处理上述命令；钉钉等
  未支持渠道回「目前支持飞书、Telegram、Slack」提示。`/help` 引导文案仅在支持渠道列出 watch 命令。
...
| 维度 | 飞书（`card_view`+`build_watch_card`） | Telegram（`telegram/watch.rs`） | Slack（`slack/watch.rs`） | 钉钉（`dingtalk/watch.rs`，PoC 已验证、M4 接入引擎） |
|---|---|---|---|---|
...
- **防刷**：每帧算**签名** =（状态、标题、最近文字、工具行、等待标志）——**不含活动时刻**：
  ...
  每卡最短编辑间隔按渠道（飞书/TG 1s、Slack 2s；tick 本身 ≥2s）。编辑失败记日志、下一拍全量重渲染
...
- **订阅持久化**：`~/.askhuman/state/watch.json`（`[{channel, sessionId, messageId, createdAt}]`，
  原子写；channel = feishu/telegram/slack）。daemon 重启 / 换新后**恢复并继续编辑同一张卡**；
...
- 新增：`src-tauri/src/watch.rs`（纯逻辑：结构化帧 / 签名 / 共享文案构件 / 持久化 / 本地时间
  ...
  `parse_watch_action`）、`telegram/router.rs::set_card_route`（仅卡回调、不认领自由文字）、
  `daemon/mod.rs` watch 引擎（`WatchClient` 传输枚举 + 按渠道路由/扰动）+ 命令分派、
  `paths::watch_file`、i18n `watch.*`。

## 后续

- 钉钉：专用 watch 模板 + PoC（高频更新实测），见 `docs/plans/im-watch-channels.md` §4。
