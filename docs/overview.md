...
AskHuman/
  ...
  src-tauri/                 Rust 后端
    ...
    src/
      ...
      watch.rs               `/watch` 实时关注纯逻辑：订阅持久化(state/watch.json) + 结构化帧/签名 +
                             共享文案构件 + 飞书卡片视图 + 渠道门控 + 本地时区绝对时刻
                             （TG/Slack 渲染在 telegram/watch.rs、slack/watch.rs；见「IM /watch」节）
...
> IM `/watch` 实时关注（spec `docs/specs/im-watch.md`，**飞书 + Telegram + Slack**；钉钉 PoC 已通过、M4 接入引擎）：`/watch <编号>`（`/关注`）关注某 agent（编号同 `/status`）——机器人发一张「实时状态卡」，此后该 agent 的进度**只就地编辑这张卡**（纯静默、零新消息）：样式化头部（eye icon + `实时关注 [n] 类型 — 项目`）+ 状态行四态（🟢 工作中 / ⚪ 空闲 / 🙋 正在等待你的回答[该 agent 有在途 AskHuman 提问时优先] / ⏹ 已结束）+ 标题 + 「最近动态（绝对时刻）」（复用 `/status` 的 transcript 尾部 × hook 实时工具融合，`autochannel::activity_parts`）+ 灰色「最后更新」行 + **动态按钮**（活动态 `[取消关注][立即刷新]`；飞书终态单个禁用按钮、TG/Slack 终态移除按钮 + 终态标签）。**帧与渲染**：`watch.rs::WatchFrame` 结构化（steps/todos，无标记语言）+ 共享文案构件；飞书 `card_view`+`build_watch_card`、Telegram `telegram/watch.rs`（HTML + inline keyboard）、Slack `slack/watch.rs`（Block Kit），差异表见 spec。**引擎**（`daemon/mod.rs::watch_*`，传输经 `WatchClient` 枚举）：AgentEvent / 提问创建 / 答复完结即时 Notify + 自适应 tick（有工作中订阅 2s、只有空闲 10s、无订阅零空转）；每帧算**签名**（结构化内容，跨渠道一致），签名不变不编辑；每卡最短编辑间隔按渠道（飞书/TG 1s、Slack 2s）；连续 5 次编辑失败自动退订；agent 结束 → 终态定格 + 自动退订。**按钮回调**按渠道路由（`WatchState::routes`）：飞书 RoutedFs ack 同步回新卡；TG `set_card_route`（仅卡回调、不抢自由文字）→ 应答 + 就地编辑；Slack `set_active(ts,"")` → 就地编辑。`/watch` 无参列当前关注、`/unwatch <编号>|all|无参`（`/取消关注`）取消（旧卡定格 + 确认文本）；重复 watch 同一 agent＝换新卡（旧卡定格「已由新卡片接替」）；上限**每渠道** 5 个、订阅/扰动/跟底水位（`WatchState::disturb`）全按渠道；未支持渠道（钉钉）回提示，help 按渠道门控（`watch::channel_supported`）。**订阅持久化** `~/.askhuman/state/watch.json`（`watch.rs`，原子写，含 channel）：daemon 重启后恢复并**继续编辑同一张卡**（seq 按 session 重解析）；有活跃订阅时 daemon 不因空闲退出（订阅随 agent 结束消亡，有界）。钉钉 PoC 已通过（`docs/plans/im-watch-channels.md` §4 结论）：专用模板 `docs/assets/dingtalk-watch-card-template.json`（11 变量，TODO 走 CollapsePanel、`finalized` 条件显隐按钮）+ 渲染器 `dingtalk/watch.rs`（`build_watch_param_map`/`parse_watch_action`，彩色 `<font colorTokenV2>` 圆点、h5 字号、NBSP 须放 font 标签内）+ 隐藏探针 `AskHuman debug dd-watch-poc`（`cli/debug_cmd.rs`，`--count 0` 样式预览）；实测 2s×150 次更新零频控 p50 ≈60–95ms。未做（M4，见 PROGRESS）：钉钉接入引擎 + 提问投放并集 watch 渠道。
>
