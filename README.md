...
# 一次提多个问题（第一题可用位置参数或 -q，后续题须用 -q；-o/-f 归属其前最近的问题）
...
多个问题时，每题以 `# Qn` 分组、题间用 `---` 分隔；某题未作答输出 `[状态]\n用户未回答此问题`，若所有题都未作答则只输出一次取消提示。单问题不加 `# Qn` 头（与上方格式一致）：
...
> 多问题：弹窗逐题展示（标题右侧 `[x/n]` 计数、底部「上一个/下一个」，全部查看过后右下角出现「提交」一次性回传）；Telegram 逐题串行发送（头部带 `[i/n]`），答完一题再发下一题，全部答完才回传。弹窗与 Telegram 同启时以「整个会话」为粒度抢答——哪端先答完全部即采用该端结果。
