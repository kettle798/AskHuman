...
pub fn help_text() -> String {
    ...
    [
        ...
        String::new(),
        "参数:".to_string(),
        "  <message>                      第一个问题的内容（必填；也可改用 -q）".to_string(),
        String::new(),
        "选项:".to_string(),
        "  -q, --question <text>          追加一个问题；第一题可用位置参数或 -q，后续题须用 -q".to_string(),
        "  -o, --option <text>            为「最近声明的问题」添加预定义选项，可多次出现".to_string(),
        "  -f, --file <path>              为「最近声明的问题」附带展示文件，可多次出现".to_string(),
...
pub fn agent_help_text() -> String {
    ...
    [
        ...
        String::new(),
        "调用方式:".to_string(),
        format!("  {prog} \"<问题>\" [-o \"<选项>\" ...] [-f \"<文件>\" ...] [-q \"<问题>\" ...] [--no-markdown]"),
        String::new(),
        "参数说明:".to_string(),
        "  <问题>                第一个问题（必填，默认按 Markdown 渲染；也可改用 -q）".to_string(),
        "  -q, --question <text> 追加一个问题；-o/-f 归属其前最近声明的问题".to_string(),
        "  -o, --option <text>   为最近的问题加预定义选项，可多次出现".to_string(),
        "  -f, --file <path>     为最近的问题附带文件或图片（绝对/相对/~），可多次出现".to_string(),
